...
class PartyMinder_Image_Manager {
    ...
    private static function validate_image_file($file, $image_type) {
        ...
        if ($file['error'] !== UPLOAD_ERR_OK) {
            return array('success' => false, 'error' => __('File upload error occurred.', 'partyminder'));
        }
        
        // Check file size
        if ($file['size'] > self::MAX_FILE_SIZE) {
            return array('success' => false, 'error' => __('Image must be smaller than 5MB.', 'partyminder'));
        }
        
        // Check file type
        if (!in_array($file['type'], self::ALLOWED_TYPES)) {
            return array('success' => false, 'error' => __('Image must be JPG, PNG, GIF, or WebP format.', 'partyminder'));
        }
        
        // Additional validation for image type
        $image_info = getimagesize($file['tmp_name']);
        if ($image_info === false) {
            return array('success' => false, 'error' => __('Invalid image file.', 'partyminder'));
        }
    ...
    private static function get_upload_directory($entity_type) {
        ...
        if (!file_exists($partyminder_dir)) {
            if (!wp_mkdir_p($partyminder_dir)) {
                return array('success' => false, 'error' => __('Failed to create upload directory.', 'partyminder'));
            }
        }
    ...
    private static function process_and_save_image($file, $file_path, $image_type) {
        ...
        switch ($image_type_const) {
            ...
            case IMAGETYPE_WEBP:
                ...
                break;
            default:
                return array('success' => false, 'error' => __('Unsupported image format.', 'partyminder'));
        }
        
        if (!$source_image) {
            return array('success' => false, 'error' => __('Failed to process image.', 'partyminder'));
        }
        ...
        if (!$save_result) {
            return array('success' => false, 'error' => __('Failed to save processed image.', 'partyminder'));
        }

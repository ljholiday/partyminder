...
class PartyMinder_Event_Manager {
    ...
    private function send_event_invitation_email($event, $inviter, $email, $token, $message = '') {
        ...
        $subject = sprintf(__('[%s] You\'re invited to %s', 'partyminder'), $site_name, $event->title);
...
The %s Team', 'partyminder'), 
    ...
    public function cancel_event_invitation($event_id, $invitation_id) {
        ...
        $result = $wpdb->update(
            ...
            array('id' => $invitation_id, 'event_id' => $event_id, 'status' => 'pending'),
            array('%s', '%s'),
            array('%d', '%d', '%s')
        );

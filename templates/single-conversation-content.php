...
$page_description = sprintf(__('Discussion in %s started by %s'), $topic->name, $conversation->author_name);
$breadcrumbs = array(
    array('title' => __('Conversations', 'partyminder'), 'url' => PartyMinder::get_conversations_url()),
    array('title' => $topic->icon . ' ' . $topic->name, 'url' => home_url('/conversations/' . $topic->slug)),
...
<div class="pm-section pm-mb">
    <div class="pm-section-header">
        <h3 class="pm-heading pm-heading-md"><?php printf(__('%d %s', 'partyminder'), $conversation->reply_count, $conversation->reply_count === 1 ? __('Reply', 'partyminder') : __('Replies', 'partyminder')); ?></h3>
    </div>

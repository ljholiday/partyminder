...
if ( $quick_rsvp && in_array( $quick_rsvp, array( 'attending', 'maybe', 'not_attending' ) ) ) {
	...
	if ( $result !== false ) {
		...
		$subject = sprintf( __( '[%1$s] Quick RSVP from %2$s', 'partyminder' ), $invitation->title, $guest_name );
		$message = sprintf(
			__( "%1\$s has quickly responded to your invitation for \"%2\$s\".\n\nResponse: %3\$s %4\$s\n\nEvent: %5\$s\nDate: %6\$s\n\nView full details: %7\$s", 'partyminder' ),
			$guest_name,
			$invitation->title,
			$guest_name,
...
if ( $_SERVER['REQUEST_METHOD'] === 'POST' && isset( $_POST['rsvp_response'] ) && wp_verify_nonce( $_POST['rsvp_nonce'], 'partyminder_guest_rsvp' ) ) {
	...
	if ( $result !== false ) {
		...
		$subject = sprintf( __( '[%1$s] RSVP Response from %2$s', 'partyminder' ), $invitation->title, $guest_name );
		$message = sprintf(
			__( "%1\$s has responded to your invitation for \"%2\$s\".\n\nResponse: %3\$s %4\$s\n\nEvent: %5\$s\nDate: %6\$s\n\nView full details: %7\$s", 'partyminder' ),
			$guest_name,
			$invitation->title,
			$guest_name,
